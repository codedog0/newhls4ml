...
class FPGABackend(Backend):
    def __init__(self, name):
        ...
        # The multiplier strategy ChoiceAttribute is shared across all multiply-having
        # layers (Dense, Conv1D, Conv2D). The user can pick any of:
        #   - standard       : exact signed multiply (Vivado infers a DSP)
        #   - lpor_k4 / k8    : Lower-Part-OR approximate multiplier (k = 4 or 8)
        #   - lsb_zero_k4/k8  : truncate bottom k bits of both operands, then exact multiply
        ...
        mult_strategy_description = (
            ...
            'partial-product quadrant with a bitwise OR ("lpor_k4"/"lpor_k8" for k=4/8); '
            'a truncated multiplier that ties the bottom k bits of both operands to '
        ...
        dense_attrs.append(
            ChoiceAttribute('mult_strategy', mult_strategy_choices, default='standard',
                            description=mult_strategy_description)
        )
        self.attribute_map[Dense] = dense_attrs

        # Conv1D / Conv2D also expose mult_strategy -- lets the user pick the
        # approximate multiplier on the last Conv layer of a CNN as well as on
        # the Dense classifier head. The vivado conv templates honor this
        # attribute the same way the Dense template does (see
        # hls4ml/backends/vivado/passes/convolution_templates.py).
        for conv_layer in (Conv1D, Conv2D):
            conv_attrs = self.attribute_map.get(conv_layer, [])
            conv_attrs.append(
                ChoiceAttribute('mult_strategy', mult_strategy_choices, default='standard',
                                description=mult_strategy_description)
            )
            self.attribute_map[conv_layer] = conv_attrs
