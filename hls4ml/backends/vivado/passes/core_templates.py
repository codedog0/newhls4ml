...
class DenseConfigTemplate(LayerConfigTemplate):
    ...
    def format(self, node):
        ...
        # Substitute in an approximate multiplier only when the default is the plain exact
        # product; binary/ternary/exponential weight paths are left untouched.
        mult_strategy = node.get_attr('mult_strategy', 'standard')
        if mult_strategy != 'standard' and params['product_type'] == 'mult':
            params['product_type'] = mult_strategy
