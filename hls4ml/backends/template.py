...
class LayerConfigTemplate(Template):
    ...
    def _default_config_params(self, layer):
        ...
        # `strategy` is interpolated directly into `nnet::{strategy}` in the config
        # templates, but the nnet::strategy enum members are lowercase (e.g. `latency`)
        # while the config convention is PascalCase (e.g. 'Latency'); normalize here so
        # every layer's generated C++ references a valid enum member.
        if isinstance(params.get('strategy'), str):
